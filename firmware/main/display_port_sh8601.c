...
#define STRIPE_ROWS 16                         /* panel rows per DMA transfer */

static esp_lcd_panel_handle_t s_panel;
static uint16_t *s_stripe;                      /* PANEL_W x STRIPE_ROWS, DMA-capable */
...
bool display_port_init(void) {
    ...
    s_stripe = heap_caps_malloc(PANEL_W * STRIPE_ROWS * 2, MALLOC_CAP_DMA);
...
void display_port_flush(const uint16_t *fb) {
    for (int py0 = 0; py0 < PANEL_H; py0 += STRIPE_ROWS) {
        for (int r = 0; r < STRIPE_ROWS; r++) {
            int py = py0 + r, x = TANK_W - 1 - py;            /* source column */
            uint16_t *dst = s_stripe + r * PANEL_W;
            for (int px = 0; px < PANEL_W; px++) {
                uint16_t c = fb[px * TANK_W + x];            /* source row = px */
                dst[px] = (uint16_t)((c << 8) | (c >> 8));
            }
        }
        esp_lcd_panel_draw_bitmap(s_panel, 0, py0, PANEL_W, py0 + STRIPE_ROWS, s_stripe);
    }
}
